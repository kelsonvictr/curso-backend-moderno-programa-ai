// Mantém o roteiro de voz alinhado aos estados visuais do teatro de contexto.
import {readFile,writeFile} from 'node:fs/promises';
import {fileURLToPath} from 'node:url';
import {spawnSync} from 'node:child_process';
const project=new URL('../../',import.meta.url);
const target=new URL('assets/audio/contexto-cap00/',project);
const raw=await readFile(new URL('capitulos/00-como-um-agente-pensa/contexto-cenas.js',project),'utf8');
const scenes=JSON.parse(raw.split('window.CONTEXTO_CENAS = ')[1].trim().replace(/;$/,''));
