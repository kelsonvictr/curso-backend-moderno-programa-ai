...
## 2026-09-19 — Fluidez do Cap 1 sem substituir a versão restaurada
- Kelson aprovou aplicar a análise incremental: prompts no ponto de uso, perguntas de negócio antes dos tipos Java, ciclos menores de hexagonal, checkpoints de integração e ajuda progressiva em AdicionarItem.
- Preservadas história, três zonas, atos, teoria, jogos, quatro pranchetas, caderno e teatro narrado. Detalhes de arquitetura continuam disponíveis em aprofundamento. Chaves de armazenamento dos alunos mantidas.
- Roteiro de 240 minutos com 15 minutos de retomada e 15 de fechamento. Revisão aceita evidência de uma decisão correta, sem exigir defeitos artificiais.
- Base em Boot 3.5.16, Java 21, Web/Validation. Data JPA/driver entram no 4B; teste de contexto é preservado como integração explícita. Dados do laboratório alinhados a c-1/CAFE-500/2 × 18.90, com UUID ilustrativo identificado e total derivado.
- Criados seis checkpoints ZIP com fonte legível, Wrapper e RETOMADA.md. Gerador extrai as specs do HTML. Não há implementação pronta de AdicionarItem. Validador cria e remove Postgres 16 isolado em porta aleatória.
- Validação executável: seis ZIPs extraídos em pastas novas compilaram e passaram seus testes (base 1, domínio 5, núcleo 8, banco 8, persistência 7, HTTP 7). Integração dos ZIPs de persistência e HTTP passou, incluindo releitura em nova transação e atualização dos itens pelo adapter.
