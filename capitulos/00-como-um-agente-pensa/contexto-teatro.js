...
(() => {
  'use strict';
  const root = document.getElementById('contexto-teatro');
  const scenes = window.CONTEXTO_CENAS;
  if (!root || !scenes) return;
  const el = name => root.querySelector('#ct-' + name);
  const tracks = window.CONTEXTO_AUDIO || {};
  ...
  function render() {
    ...
    set('capacity-label',s.load + ' / 8 unidades ilustrativas');
    el('capacity').replaceChildren();
    for (let i=0;i<8;i++) { const bar=document.createElement('i');bar.classList.toggle('on',i<s.load);bar.style.animationDelay=(i*.035)+'s';el('capacity').append(bar); }
    ...
    root.querySelectorAll('[data-ct-scene]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.ctScene)===scene)));
  }
  function schedule() {
    const id=++generation;
    ...
    const next=()=>{
      ...
      const current=new Audio('../../assets/audio/contexto-cap00/audio/'+clip.file);
      current.hidden=true;current.dataset.contextoAudio='true';root.append(current);audio=current;
      let settled=false;
  ...
  root.querySelectorAll('[data-ct-scene]').forEach(b=>b.addEventListener('click',()=>{scene=Number(b.dataset.ctScene);reset();}));
